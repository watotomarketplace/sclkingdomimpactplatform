...
export default async function FacilitatorRedFlagsPage() {
  ...
  const redFlags = pods.flatMap(pod =>
    pod.members
      .map(({ user }) => {
        const scorecards = user.scorecards;
        const isRedFlag = scorecards.length >= 3 && scorecards.every(sc => getOverallStatus(sc) === "ESCALATE");
  ...
  return (
    <div className="px-6 py-6 max-w-[780px]">
      <div className="mb-6">
        <h1 className="font-display text-[28px] font-semibold text-text-primary">Red Flags</h1>
        <p className="text-text-secondary text-sm mt-1">Participants with 3+ consecutive Escalate scorecards</p>
      </div>
